...
export class ToggleTodoUseCase implements IUseCase<ToggleTodoInput, ToggleTodoOutput> {
    public constructor(private readonly repository: ITodoRepository) { }

    async execute({ id, done }: ToggleTodoInput): Promise<ApplicationException | ToggleTodoOutput> {
